...
public class File_manager implements ActionListener, ClipboardOwner {
    ...
    private int[] copy;
    private String[] Korean = {"파일 이름", "파일 크기", "마지막으로 수정한 날짜"};
    private String[] English = {"Name", "Size", "Modified"};
    ...
    public File_manager() {
        ...
        PopItem_Korea[0] = new JMenuItem("폴더로보기");
        PopItem_Korea[1] = new JMenuItem("복사하기");
        PopItem_Korea[2] = new JMenuItem("붙여넣기");
        PopItem_Korea[3] = new JMenuItem("삭★제");
        PopItem_English[0] = new JMenuItem("Shows with Folder");
        ...
        sp2.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                if (SwingUtilities.isRightMouseButton(e)) {
                    ...
                    PopMenu.show(jt1, e.getX(), e.getY());
        ...
        sp2.setViewportView(jt1);
        jc.add(jl,BorderLayout.WEST);
        jc.add(comboBox1,BorderLayout.EAST);
        ju.add(sp2, BorderLayout.CENTER);
        ju.add(sp1,BorderLayout.WEST);
        p1.add(homeTextField, BorderLayout.NORTH);
        p1.add(jc, BorderLayout.SOUTH);
        p1.add(ju, BorderLayout.CENTER);
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == PopItem_Korea[3] || e.getSource() == PopItem_English[3]) {
            int[] columns = jt1.getSelectedRows();
            for (int column : columns) {
                System.out.println(column);
                System.out.println(path + File.separator + jt1.getValueAt(column, 0));
            }
            for (int column : columns) {
                File delete = new File(path + File.separator + jt1.getValueAt(column, 0));
                delete.delete();
            }
        ...
        if (e.getSource() == PopItem_Korea[1] || e.getSource() == PopItem_English[1]) {
            ...
            copy = jt1.getSelectedRows();
            copyFile = new Vector<>(jt1.getRowCount());
            for (int i = 0; i < copy.length; i++) {
                copyFile.add(i, (path + "\\" + jt1.getValueAt(copy[i], 0)));
            }
        }
        ...
        if (e.getSource() == PopItem_Korea[2] || e.getSource() == PopItem_English[2]) {
            ...
            for (int i = 0; i < copy.length; i++) {
                ...
                try {
                    ...
                    if (result.toString().contains("0개 파일 복사(애걔?)")) {
                        if (comboBox1.getSelectedItem() == "한글")
                            JOptionPane.showMessageDialog(null, "액세스 오류 발생(권한 없음)", "에러", JOptionPane.ERROR_MESSAGE);
                        else
    ...
    private void getJList() {
        ...
        if (file_list != null) {
            ...
            if (comboBox1.getSelectedItem() == "한글")
                JOptionPane.showMessageDialog(null, "액세스 오류(권한없음)", "에러", JOptionPane.ERROR_MESSAGE);
    ...
    private void setTable() {
        if (comboBox1.getSelectedItem() == "한글") {
            model = new DefaultTableModel(data, Korean);
            jt1.setModel(model);
            jl.setText("파일 매니저");
        }
        if (comboBox1.getSelectedItem() == "English") {
            model = new DefaultTableModel(data, English);
            jt1.setModel(model);
